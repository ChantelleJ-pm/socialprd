# Recipe Clipper Stakeholder Slides

## Slide 1 - Recipe Clipper is improving on key leading KPIs

### Slide title
Recipe Clipper is improving on key leading KPIs

### Slide subtitle
The Feb 9 support-list expansion and the cleaner mid-Feb reporting period both point to a healthier core save flow.

### On-slide callouts

- **52% success rate post-mid-Feb**
  - Up from **44%** in Feb 1-8 to **50%** in Feb 9-18
- **3.8s average save time post-mid-Feb**
  - Improved from **5.2s** in Feb 1-8 to **4.3s** in Feb 9-18
- **85% of successful saves are external-brand URLs**
  - Confirms the feature is being used for its intended external clipping use case

### Supporting bullets

- After adding **126 supported sites on Feb 9**, save-click-to-success conversion improved from **47% to 54%**
- User error rate fell from **35% to 25% of submissions** in the immediate pre/post Feb 9 window
- These are strong leading indicators that the experience is becoming faster, more reliable, and more valuable for external recipe saving

### Speaker notes

- Use **2/19 onward** as the cleanest read on error attribution because the save URL error categories were updated around mid-Feb.
- We should frame this as **progress on leading KPIs** we can see in the export: success rate, time to save, and external recipe adoption.
- We **cannot yet claim** progress on collection creation or repeat sessions from this export because those metrics are not included here.

## Slide 2 - Remaining friction is mostly user and site coverage, not core product reliability

### Slide title
Remaining friction is mostly user and site coverage, not core product reliability

### Slide subtitle
Post-mid-Feb, product-side issues are the smallest of the three main friction buckets.

### On-slide callouts

- **Outcome mix of save attempts, post-mid-Feb**
  - **52%** success
  - **25%** user-side errors
  - **13%** external/domain errors
  - **10%** product-side errors

- **92% of external errors are unsupported-site related**
  - The planned **300+ additional sites** is the right next lever

- **84% of product errors come from general_fail + bookmark_fail**
  - Product cleanup can stay focused and targeted

### Supporting bullets

- Do **not** headline product issues as "20%"; that figure is the share of failures, not the share of attempts
- The clearer stakeholder message is that the core product is working for a majority of attempts, while the biggest remaining scaling constraint is site coverage
- The next best moves are:
  - add the next 300+ supported sites
  - keep reducing user input friction
  - continue targeted cleanup of general and bookmark failures

### Speaker notes

- If asked about the "21%" number, clarify that product issues are about **10% of save attempts** and about **21% of classified failures**.
- This keeps the conversation accurate and prevents stakeholders from inferring that the core product is broadly broken.
- The strategic takeaway is that domain expansion is now a data-backed growth and reliability lever, not just backlog maintenance.
